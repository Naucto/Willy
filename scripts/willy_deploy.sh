...
env_get() { [ -f "$WILLY_DIR/.env" ] && sed -n "s/^$1=//p" "$WILLY_DIR/.env" | head -1 || true; }
...
EOF
  ...
  # Add a small swap file if the box has none and little RAM.
...
write_env() {
  ...
  master_key="$(env_get WILLY_MASTER_KEY)";       [ -n "$master_key" ] || master_key="$(gen_secret 32)"
  jwt_secret="$(env_get JWT_SECRET)";             [ -n "$jwt_secret" ] || jwt_secret="$(gen_secret 32)"
  jwt_refresh="$(env_get JWT_REFRESH_SECRET)";    [ -n "$jwt_refresh" ] || jwt_refresh="$(gen_secret 32)"
  pg_pass="$(env_get POSTGRES_PASSWORD)";         [ -n "$pg_pass" ] || pg_pass="$(gen_secret 16)"
...
WILLY_ADMIN_PASSWORD=${WILLY_ADMIN_PASSWORD:-$(gen_secret 12)}
EOF
  chmod 600 "$tmp"
...
configure_traefik() {
  ...
  [ -n "$email" ] || die "ACME_EMAIL is not set and not found in .env"

  ca="$ACME_STAGING"
  [ -f "$WILLY_DIR/.acme-prod" ] && ca="$ACME_PROD"
  [ "${ACME_CA:-}" = "prod" ] && { ca="$ACME_PROD"; touch "$WILLY_DIR/.acme-prod"; chown "$WILLY_USER:$WILLY_USER" "$WILLY_DIR/.acme-prod"; }
...
generate_dashboard_auth() {
  ...
  if [ ! -f "$WILLY_DIR/routing/auth/dashboard.htpasswd" ]; then
    pass="$(env_get WILLY_ADMIN_PASSWORD)"; [ -n "$pass" ] || pass="$(gen_secret 12)"
...
issue_cert() {
  ...
  if [ ! -f "$WILLY_DIR/.acme-prod" ]; then
    ...
    chmod 600 "$WILLY_DIR/routing/letsencrypt/acme.json"
    touch "$WILLY_DIR/.acme-prod"; chown "$WILLY_USER:$WILLY_USER" "$WILLY_DIR/.acme-prod"
    configure_traefik
    as_willy "cd '$WILLY_DIR' && docker compose up -d --force-recreate traefik"
    i=0
    while [ "$i" -lt 30 ]; do acme_has_cert && break; i=$((i + 1)); sleep 6; done
  fi
...
cmd_upgrade() {
  require_root
  [ -d "$WILLY_DIR/.git" ] || die "Willy is not installed at $WILLY_DIR"
  clone_or_update_repo
  configure_traefik
  prepare_acme
...
check() { # check "<label>" <0|1 ok?> ; sets rc=1 on failure
  if [ "$2" -eq 0 ]; then
    echo "  [ok] $1"
  else
    echo "  [!!] $1"
...
cmd_doctor() {
  ...
  if [ -f "$WILLY_DIR/.env" ]; then
    [ "$(stat -c '%a' "$WILLY_DIR/.env")" = "600" ]; check ".env present (600)" "$?"
  else
    check ".env present" 1
  fi

  if [ -f "$WILLY_DIR/routing/letsencrypt/acme.json" ]; then
    [ "$(stat -c '%a' "$WILLY_DIR/routing/letsencrypt/acme.json")" = "600" ]; check "acme.json present (600)" "$?"
    if acme_has_cert; then echo "  [ok] certificate present"; else echo "  [..] no certificate yet"; fi
  else
