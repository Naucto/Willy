...
# --- Parse args ---
case "${1:-}" in provision|upgrade|doctor|backup) VERB="$1"; shift ;; esac
while [ $# -gt 0 ]; do
  case "$1" in
    --host) HOST="$2"; shift 2 ;;
...
if [ -n "$BASE_DOMAIN" ]; then
  if [ -n "$PANEL_SUBDOMAIN" ]; then PANEL_HOST="$PANEL_SUBDOMAIN.$BASE_DOMAIN"; else PANEL_HOST="$BASE_DOMAIN"; fi
else
  PANEL_HOST=""
fi
...
preflight() {
  ...
  if [ "$VERB" = "provision" ]; then
    [ -n "$BASE_DOMAIN" ] || die "--base-domain is required for provision"
    [ -n "$ACME_EMAIL" ] || die "--acme-email is required for provision"
    [ -n "${OVH_APPLICATION_KEY:-}" ] && [ -n "${OVH_APPLICATION_SECRET:-}" ] && [ -n "${OVH_CONSUMER_KEY:-}" ] \
...
$( [ -n "$REPO" ] && echo "export WILLY_REPO='$REPO'" )
$( [ -n "$CA" ] && echo "export ACME_CA='$CA'" )
EOF
}
...
[ "$VERB" = "provision" ] && [ -n "$PANEL_HOST" ] && log "Done. Panel: https://$PANEL_HOST"
exit 0
